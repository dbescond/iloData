...
remDr$navigate('http://data.imf.org')
	
	Sys.sleep(20)

	webElem <- remDr$findElement('id', 'EddHeaderLogIn')
	new <- webElem$findChildElements(using = 'css selector', value = "div")
	
	new[[7]]$highlightElement()	
	new[[7]]$clickElement()	
		Sys.sleep(20)

	test <- remDr$findElement('id', 'username')
	test$highlightElement()	
	
	test$sendKeysToElement(list('bescond@ilo.org'))
	
		test <- remDr$findElement('id', 'password')
	test$highlightElement()	
	
	test$sendKeysToElement(list('Ilostat123+++'))
	
		
	test <- remDr$findElement('id', 'update')
	test$clickElement()	
	Sys.sleep(20)
	...
	test <- remDr$findElement('id', "TableContent")
	test$highlightElement()	
		Sys.sleep(80)
	
		new <- test$findChildElement(value = '//li[@class = "BDItem BDInline BDRestartExport"]')
		new$highlightElement()	
	
		try(new$clickElement(), silent = TRUE)	
		Sys.sleep(200)
	
	
		
		new <- test$findChildElement(value = '//li[@class = "BDItem BDInline BDDownloadExport"]')
...
rm(remDr, webElem, new, action, test, fprof, AllWindow) 
...
if(length(test)>1){
	...
	unlink(paste0("C:/temp/", test[i]))
	}
}
file.rename(paste0("C:\\temp\\",test[1]),paste0('input/IMF_CPI.zip'))

keep <- unzip(paste0('input/IMF_CPI.zip'), list = TRUE) %>% filter(str_sub(Name, 1,3) %in% 'CPI') %>% select(Name) %>% t %>% as.character

setwd('./input/')
unzip('IMF_CPI.zip', keep)


file.rename(keep, 'IMF_CPI.csv')
