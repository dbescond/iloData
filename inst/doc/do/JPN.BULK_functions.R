...
download_data_JPN <- function(Mapping_File){
  ...
  action <- remDr$open()   
  URL <- 'https://www.e-stat.go.jp/en/dbview?sid=0002060002'
  ...
  ItemSelector <- remDr$findElement(value = '//li[@data-display = "js-dbview-display_selector"]')
  
  
  ItemSelector$highlightElement()
    ItemSelector$clickElement()
  
  
  
  
   <- webElem[[1]]$findChildElements(value = '//li[@data-display = "js-dbview-display_selector"]')
