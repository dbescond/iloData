...
REP_OECD.LFS_ANNUAL_input <- function(){	# DRIVER
require(ilo)		

setwd(paste0(ilo:::path$data, '/REP_OECD/LFS_ANNUAL/'))
			
  source(paste0(getwd(),"/do/SOURCE_OECD_LFS_ANNUAL_DEFINE.r"))
  ...
  res <-	bind_rows( 	readRDS('./tmp/ANNUAL_UNE_AGE_DUR.rds'), 
						readRDS('./tmp/ANNUAL_AGE.rds'),
						readRDS('./tmp/ANNUAL_STE.rds'), 
						readRDS('./tmp/ANNUAL_EMP_SEX_ECO_ISIC3.rds'),
						readRDS('./tmp/ANNUAL_EMP_SEX_ECO_ISIC4.rds')) %>% 
...
REP_OECD.LFS_ANNUAL_input_ANNUAL_EMP_SEX_ECO_ISIC3 <- function()  { # Employment and Employees by Eco Activity, ISIC3
	...
	input_path	<- paste0(getwd(),'/input/ANNUAL_EMP_SEX_ECO_ISIC3_TEST.csv')
  ...
  Xpp	<-	SOURCE_OECD_LFS_ANNUAL_DEFINE(Xpp)
...
saveRDS(Xpp,file = './tmp/ANNUAL_EMP_SEX_ECO_ISIC3.rds' )
#	X <- readRDS('./tmp/ANNUAL_EMP_SEX_ECO_ISIC3.rds')
...
REP_OECD.LFS_ANNUAL_input_ANNUAL_EMP_SEX_ECO_ISIC4 <- function() { # Employment and Employees by Eco Activity, ISIC4
  ...
  input_path<- paste0(getwd(),'/input/ANNUAL_EMP_SEX_ECO_ISIC4_TEST.csv')
  ...
  ### STEP 6 Exporting
Xpp$time <- as.character(Xpp$time) 
saveRDS(Xpp,file = './tmp/ANNUAL_EMP_SEX_ECO_ISIC4.rds' )
#	X <- readRDS('./tmp/ANNUAL_EMP_SEX_ECO_ISIC4.rds')
...
REP_OECD.LFS_ANNUAL_input_ANNUAL_ANNUAL_AGE_DUR <- function(){ # processing of "UNE_TUNE_SEX_AGE_DUR_NB
  ...
  ANNUAL_UNE_AGE_DUR <- left_join(	ANNUAL_UNE_AGE_DUR, 
                                   readRDS('./tmp/ANNUAL_AGE.rds') %>% filter(indicator %in% 'UNE_TUNE_SEX_AGE_NB') %>% distinct(ref_area, indicator, time, source, note_source), by = c('ref_area', 'indicator', 'time')) %>% 
...
REP_OECD.LFS_ANNUAL_input_ANNUAL_ANNUAL_EMP_STE <- function(){	# processing of EMP_TEMP_SEX_STE_NB
  ...
  REF <- readRDS('./tmp/ANNUAL_AGE.rds')  %>% filter(indicator %in% 'EMP_TEMP_SEX_AGE_NB') %>% distinct(ref_area, source,note_source) %>% group_by(ref_area, source ) %>% summarise(note_source = last(note_source)) %>% ungroup 
...
SOURCE_OECD_LFS_ANNUAL_DEFINE <- function (DataWithoutSource) { # Utility to get source and note_source- with cleaning (Specific to OECD data)
